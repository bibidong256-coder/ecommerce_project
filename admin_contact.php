...
if (isset($_GET['mark_read'])) {
    $conn->prepare("UPDATE contact_messages SET is_read = 1 WHERE id = ?")->execute([(int)$_GET['mark_read']]);
    header('Location: admin_dashboard.php?tab=messages'); exit;
}
if (isset($_GET['mark_unread'])) {
    $conn->prepare("UPDATE contact_messages SET is_read = 0 WHERE id = ?")->execute([(int)$_GET['mark_unread']]);
    header('Location: admin_dashboard.php?tab=messages'); exit;
}
if (isset($_GET['delete_msg'])) {
    $conn->prepare("DELETE FROM contact_messages WHERE id = ?")->execute([(int)$_GET['delete_msg']]);
    header('Location: admin_dashboard.php?tab=messages'); exit;
}
if (isset($_GET['delete_sub'])) {
    $conn->prepare("DELETE FROM newsletter_subscribers WHERE id = ?")->execute([(int)$_GET['delete_sub']]);
    header('Location: admin_dashboard.php?tab=subscribers'); exit;
}
if (isset($_GET['toggle_sub'])) {
    $conn->prepare("UPDATE newsletter_subscribers SET is_active = NOT is_active WHERE id = ?")->execute([(int)$_GET['toggle_sub']]);
    header('Location: admin_dashboard.php?tab=subscribers'); exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_staff'])) {
    ...
    if ($id > 0) {
        $conn->prepare("UPDATE contact_staff SET name=?,role=?,phone=?,email=?,photo=?,sort_order=? WHERE id=?")
             ->execute([$name,$role,$phone,$email,$photo,$order,$id]);
    } else {
        $conn->prepare("INSERT INTO contact_staff (name,role,phone,email,photo,sort_order) VALUES (?,?,?,?,?,?)")
             ->execute([$name,$role,$phone,$email,$photo,$order]);
    }
...
if (isset($_GET['delete_staff'])) {
    $conn->prepare("DELETE FROM contact_staff WHERE id = ?")->execute([(int)$_GET['delete_staff']]);
    header('Location: admin_dashboard.php?tab=contact_page'); exit;
}

// ── Toggle staff active ─────────────────────────────
if (isset($_GET['toggle_staff'])) {
    $conn->prepare("UPDATE contact_staff SET is_active = NOT is_active WHERE id = ?")->execute([(int)$_GET['toggle_staff']]);
    header('Location: admin_dashboard.php?tab=contact_page'); exit;
}
...
if ($search) {
    $like = "%$search%";
    $stmt = $conn->prepare("SELECT * FROM contact_messages WHERE name LIKE ? OR email LIKE ? OR subject LIKE ? OR message LIKE ? ORDER BY submitted_at DESC");
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root {
            --primary:#088178; --dark:#04534e; --accent:#e8f5f4;
            --bg:#f4f6f8; --white:#fff; --text:#1a1a2e;
        ...
        /* ── Alert ── */
        .alert { padding:12px 18px; border-radius:10px; margin-bottom:24px; font-size:.9rem; font-weight:600; }
        ...
        .search-bar input { flex:1; padding:11px 16px; border:2px solid var(--border); border-radius:var(--radius); font-size:.9rem; font-family:inherit; outline:none; transition:border-color .2s; min-width:200px; }
        .search-bar input:focus { border-color:var(--primary); }
        .search-bar button { padding:11px 20px; background:var(--primary); color:#fff; border:none; border-radius:var(--radius); font-size:.88rem; font-weight:600; font-family:inherit; cursor:pointer; transition:background .2s; }
        .search-bar button:hover { background:var(--dark); }
        .search-bar a { padding:11px 16px; background:#f3f4f6; color:var(--muted); border-radius:var(--radius); font-size:.88rem; font-weight:600; text-decoration:none; }

        /* ── Stats ── */
        .stats { display:grid; grid-template-columns:repeat(auto-fit,minmax(180px,1fr)); gap:20px; margin-bottom:36px; }
        .stat-card { background:var(--white); border-radius:var(--radius); padding:24px 22px; display:flex; align-items:center; gap:18px; box-shadow:0 2px 10px rgba(0,0,0,.05); border:1.5px solid var(--border); }
        ...
        .stat-card h3 { font-size:1.7rem; font-weight:700; line-height:1; }
        .stat-card p  { font-size:.82rem; color:var(--muted); margin-top:4px; }
        ...
        .card-header { display:flex; align-items:center; justify-content:space-between; padding:20px 24px; border-bottom:1.5px solid var(--border); flex-wrap:wrap; gap:8px; }
        ...
        thead th { text-align:left; padding:13px 20px; font-size:.78rem; font-weight:700; text-transform:uppercase; letter-spacing:.8px; color:var(--muted); border-bottom:1.5px solid var(--border); }
        tbody tr { border-bottom:1px solid var(--border); transition:background .15s; }
        tbody tr:last-child { border-bottom:none; }
        tbody tr:hover { background:#fafafa; }
        tbody tr.unread { background:#f0fdf9; }
        tbody td { padding:14px 20px; font-size:.88rem; vertical-align:top; }
        .badge-unread { display:inline-block; background:#088178; color:#fff; font-size:.68rem; font-weight:700; padding:2px 8px; border-radius:20px; margin-left:6px; vertical-align:middle; }
        .msg-text { max-width:260px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; color:var(--muted); font-size:.83rem; cursor:pointer; }
        .msg-text:hover { color:var(--primary); text-decoration:underline; }

        /* ── Buttons ── */
        .actions { display:flex; gap:6px; flex-wrap:wrap; }
        .btn-sm { display:inline-flex; align-items:center; gap:5px; padding:6px 11px; border-radius:7px; font-size:.76rem; font-weight:600; font-family:inherit; text-decoration:none; cursor:pointer; border:none; transition:opacity .2s,transform .15s; white-space:nowrap; }
        ...
        .form-group label { font-size:.83rem; font-weight:700; color:#444; letter-spacing:.3px; }
        .form-group input,
        .form-group textarea { padding:11px 14px; border:2px solid var(--border); border-radius:10px; font-size:.9rem; font-family:inherit; outline:none; transition:border-color .2s; color:var(--text); }
        .form-group input:focus,
        .form-group textarea:focus { border-color:var(--primary); box-shadow:0 0 0 4px rgba(8,129,120,.08); }
        .form-group textarea { min-height:80px; resize:vertical; }
        .btn-save { display:inline-flex; align-items:center; gap:8px; padding:13px 28px; background:var(--primary); color:#fff; border:none; border-radius:var(--radius); font-size:.95rem; font-weight:700; font-family:inherit; cursor:pointer; transition:background .2s,transform .2s; }
        .btn-save:hover { background:var(--dark); transform:translateY(-2px); }

        /* ── Staff section ── */
        .section-title { font-family:'Syne',sans-serif; font-size:1rem; font-weight:700; margin-bottom:16px; padding-bottom:10px; border-bottom:2px solid var(--accent); color:var(--text); }
        .staff-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(280px,1fr)); gap:16px; margin-bottom:24px; }
        .staff-card { background:#f9fafb; border:1.5px solid var(--border); border-radius:var(--radius); padding:18px; display:flex; gap:14px; align-items:flex-start; transition:border-color .2s; }
        .staff-card:hover { border-color:var(--primary); }
        .staff-card img { width:56px; height:56px; border-radius:50%; object-fit:cover; border:2px solid var(--accent); flex-shrink:0; }
        .staff-card .info h4 { font-size:.95rem; font-weight:700; color:var(--text); margin-bottom:3px; }
        .staff-card .info p  { font-size:.8rem; color:var(--muted); line-height:1.5; }
        .staff-card .info .actions { margin-top:10px; }
        ...
        .modal { background:#fff; border-radius:var(--radius); padding:32px; max-width:560px; width:100%; box-shadow:0 20px 60px rgba(0,0,0,.2); position:relative; max-height:90vh; overflow-y:auto; }
        ...
        .modal .body { font-size:.92rem; line-height:1.7; color:var(--text); white-space:pre-wrap; background:#f9fafb; padding:16px; border-radius:8px; border:1px solid var(--border); }
...
<div class="layout">
    ...
    <aside class="sidebar">
        ...
        <nav>
          <a href="admin_dashboard.php" class="<?= $active_tab === 'admin_dashboard' ? 'active' : '' ?>">
    ...
    <main class="main">
        <div class="topbar">
            <h1>
                <?php
                if ($active_tab==='messages') echo 'Contact Messages';
                elseif ($active_tab==='subscribers') echo 'Newsletter Subscribers';
                else echo 'Edit Contact Page';
                ?>
            </h1>
        ...
        <div class="<?= $active_tab!=='messages'?'tab-hidden':'' ?>">
            <form class="search-bar" method="GET">
                ...
                <?php if($search): ?><a href="?tab=messages"><i class="fas fa-times"></i> Clear</a><?php endif; ?>
            </form>
            <div class="card">
                ...
                <?php if(empty($messages)): ?>
                    <div class="empty"><i class="fas fa-inbox"></i><p><?= $search?'No messages match your search.':'No messages yet.' ?></p></div>
                <?php else: ?>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr class="<?= !$msg['is_read']?'unread':'' ?>">
                            ...
                            <td><div class="msg-text" onclick="openModal('<?= htmlspecialchars(addslashes($msg['name'])) ?>','<?= htmlspecialchars(addslashes($msg['email'])) ?>','<?= htmlspecialchars(addslashes($msg['subject']?:'(No subject)')) ?>','<?= htmlspecialchars(addslashes($msg['message'])) ?>','<?= date('d M Y, H:i',strtotime($msg['submitted_at'])) ?>')" title="Click to read"><?= htmlspecialchars($msg['message']) ?></div></td>
                            ...
                            <td>
                                <div class="actions">
                                    ...
                                    <?php endif; ?>
                                    <a href="?delete_msg=<?=$msg['id']?>&tab=messages" class="btn-sm btn-delete" onclick="return confirm('Delete this message?')"><i class="fas fa-trash"></i> Delete</a>
                                </div>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span style="display:inline-block;padding:3px 10px;border-radius:20px;font-size:.75rem;font-weight:700;background:<?=$sub['is_active']?'#e8f5f4':'#f3f4f6'?>;color:<?=$sub['is_active']?'#088178':'#6b7280'?>;"><?=$sub['is_active']?'Active':'Inactive'?></span></td>
                            ...
                            <td>
                                <div class="actions">
                                    ...
                                    <a href="?delete_sub=<?=$sub['id']?>&tab=subscribers" class="btn-sm btn-delete" onclick="return confirm('Remove this subscriber?')"><i class="fas fa-trash"></i> Remove</a>
                                </div>
                            </td>
                        </tr>
                        <?php endforeach; ?>
        ...
        <div class="<?= $active_tab!=='contact_page'?'tab-hidden':'' ?>">
                ...
                <form method="POST" class="settings-form">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            <label>Page Title (big heading)</label>
                            <input type="text" name="page_title" value="<?= htmlspecialchars($cs['page_title'] ?? '') ?>" placeholder="#Let's_Talk">
                        </div>
                        <div class="form-group">
                            <label>Page Subtitle</label>
                            <input type="text" name="page_subtitle" value="<?= htmlspecialchars($cs['page_subtitle'] ?? '') ?>" placeholder="LEAVE A MESSAGE...">
                        </div>
                        <div class="form-group full">
                            <label>Address</label>
                            <input type="text" name="address" value="<?= htmlspecialchars($cs['address'] ?? '') ?>" placeholder="56 Kaguje Street, Kampala, Uganda">
                        </div>
                        <div class="form-group">
                            <label>Phone 1</label>
                            <input type="text" name="phone1" value="<?= htmlspecialchars($cs['phone1'] ?? '') ?>">
                        </div>
                        <div class="form-group">
                            <label>Phone 2 (optional)</label>
                            <input type="text" name="phone2" value="<?= htmlspecialchars($cs['phone2'] ?? '') ?>">
                        </div>
                        <div class="form-group">
                            <label>Email Address</label>
                            <input type="email" name="email" value="<?= htmlspecialchars($cs['email'] ?? '') ?>">
                        </div>
                        <div class="form-group">
                            <label>Business Hours</label>
                            <input type="text" name="hours" value="<?= htmlspecialchars($cs['hours'] ?? '') ?>" placeholder="Mon-Sat: 10AM - 8PM">
                        </div>
                        <div class="form-group full">
                            <label>Google Maps Embed URL</label>
                            <p style="font-size:.78rem;color:var(--muted);margin-bottom:6px;">Go to Google Maps → Share → Embed a map → copy the <strong>src="..."</strong> URL only</p>
                            <textarea name="map_embed_url" placeholder="https://www.google.com/maps/embed?..."><?= htmlspecialchars($cs['map_embed_url'] ?? '') ?></textarea>
                        </div>
                    </div>
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="#" onclick="openStaffModal(0)" class="btn-sm btn-read"><i class="fas fa-plus"></i> Add Staff</a>
                </div>
                <div style="padding:24px;">
                    <?php if(empty($staff_list)): ?>
                        <div class="empty" style="padding:20px 0;"><i class="fas fa-user-slash"></i><p>No staff members yet. Click "Add Staff".</p></div>
                    <?php else: ?>
                    <div class="staff-grid">
                        <?php foreach($staff_list as $member): ?>
                        <div class="staff-card">
                            <img src="<?= htmlspecialchars($member['photo']) ?>" alt="<?= htmlspecialchars($member['name']) ?>"
                                 onerror="this.src='https://ui-avatars.com/api/?name=<?= urlencode($member['name']) ?>&background=088178&color=fff'">
                            <div class="info">
                                ...
                                <p style="margin-top:5px;">
                                    <span style="display:inline-block;padding:2px 8px;border-radius:20px;font-size:.72rem;font-weight:700;background:<?=$member['is_active']?'#e8f5f4':'#f3f4f6'?>;color:<?=$member['is_active']?'#088178':'#6b7280'?>;">
                                        <?=$member['is_active']?'Visible':'Hidden'?>
                                    </span>
                                    · Order: <?=$member['sort_order']?>
                                </p>
                                <div class="actions" style="margin-top:10px;">
                                    <a href="#" onclick="openStaffModal(<?=$member['id']?>,<?= htmlspecialchars(json_encode($member)) ?>)" class="btn-sm btn-edit"><i class="fas fa-pen"></i> Edit</a>
                                    <a href="?toggle_staff=<?=$member['id']?>&tab=contact_page" class="btn-sm btn-toggle"><i class="fas fa-eye<?=$member['is_active']?'':'-slash'?>"></i> <?=$member['is_active']?'Hide':'Show'?></a>
                                    <a href="?delete_staff=<?=$member['id']?>&tab=contact_page" class="btn-sm btn-delete" onclick="return confirm('Delete this staff member?')"><i class="fas fa-trash"></i></a>
                                </div>
                            </div>
                        </div>
                        <?php endforeach; ?>
        ...
        <form method="POST" style="margin-top:16px;">
            ...
            <div class="form-grid" style="grid-template-columns:1fr 1fr;gap:14px;">
                <div class="form-group">
                    ...
                    <input type="text" name="staff_name" id="staff_name" required placeholder="e.g. NAKIMULI DIANA">
                </div>
                <div class="form-group">
                    <label>Role / Title</label>
                    <input type="text" name="staff_role" id="staff_role" placeholder="e.g. Senior Accounts Manager">
                </div>
                <div class="form-group">
                    <label>Phone</label>
                    <input type="text" name="staff_phone" id="staff_phone" placeholder="+256 757 449 384">
                </div>
                <div class="form-group">
                    <label>Email</label>
                    <input type="email" name="staff_email" id="staff_email" placeholder="email@example.com">
                </div>
                <div class="form-group" style="grid-column:1/-1;">
                    <label>Photo Path (relative to your site root)</label>
                    <input type="text" name="staff_photo" id="staff_photo" placeholder="officials/photo.jpg">
                </div>
                <div class="form-group">
                    <label>Display Order</label>
                    <input type="number" name="staff_order" id="staff_order" value="0" min="0">
                </div>
            </div>
            <button type="submit" class="btn-save" style="margin-top:16px;"><i class="fas fa-save"></i> Save Staff</button>
        </form>
    </div>
</div>
...
function openStaffModal(id, data) {
    document.getElementById('staffModalTitle').textContent = id > 0 ? 'Edit Staff Member' : 'Add Staff Member';
